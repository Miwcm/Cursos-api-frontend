...
type ComboboxAnchor = 'top' | 'bottom'
...
type ComboboxProps<T> = ComboboxBaseProps<T> &
    Omit<
...
export function Combobox<T>({
    ...
    return (
        <Headless.Combobox<T>
            ...
            multiple={false}
            value={internalValue}
            onChange={(v) => onChange(v ?? null)} // normalize back to null
...
export function ComboboxOption<T>({
    ...
    return (
        ...
        >
      <span className={clsx(className, 'flex min-w-0 items-center')}>
        {children}
      </span>
            <svg
